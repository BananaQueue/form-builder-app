 ...
 return (
  <div className="fb-shell">
                    ...
                    <>
                      <div className="fb-field" style={{ marginBottom: 0 }}>
                        ...
                        <select
                          className="fb-select"
                          value={conditionType}
                          onChange={(e) => { setConditionType(e.target.value); setConditionValue('') }}
                        >
                          ...
                          {(selectedQ.type === 'checkbox' || selectedQ.type === 'rating') && (
                            <option value="option_selected">Specific option is selected</option>
                          )}
                      ...
                      {conditionType !== 'is_answered' && (
                        <div className="fb-field" style={{ marginBottom: 0 }}>
                          <label className="fb-label">
                            {conditionType === 'option_selected' ? 'Option' : 'Value'}
                          </label>
                          {selectedQ.type === 'text' ? (
                            <input
                              className="fb-input"
                      ...
                      <p className="fb-condition-hint" style={{ margin: 0 }}>
                        {conditionType === 'equals' && 'Shows when answer exactly matches the value above.'}
                        {conditionType === 'not_equals' && 'Shows when answer does NOT match the value above.'}
                        {conditionType === 'option_selected' && 'Shows when the specific checkbox option is checked.'}
                        {conditionType === 'is_answered' && 'Shows when the previous question has any answer.'}
                      </p>
                    </>
                  )
                })()}
      ...
      {(newQuestionType === 'multiple_choice' || newQuestionType === 'checkbox') && (
        <div className="fb-options-panel">
          ...
          <div className="fb-add-option-row">
            ...
            />
            <button className="fb-btn-add" style={{ width: 'auto', padding: '10px 18px' }} onClick={addOption}>
              Add
            </button>
          </div>
          {tempOptions.length > 0 && (
            ...
            <>
              ...
              <div className="fb-add-option-row">
                ...
                <button
                  ...
                  onClick={() => {
                    if (newOption.trim() === '') { alert('Please enter an option'); return }
                  ...
                  {customRatingOptions.map((option, index) => (
                    <div key={index} className="fb-option-pill">
                      ...
                      <button
                        ...
                        onClick={() => setCustomRatingOptions(customRatingOptions.filter((_, i) => i !== index))}
                      >×</button>
                    </div>
                  ))}
                </div>
    ...
    <div className="fb-save-panel">
      ...
      <p className="fb-save-count">Your form has {questions.length} question{questions.length !== 1 ? 's' : ''}</p>
    ...
    <details className="fb-debug">
      ...
      <pre>{JSON.stringify({ title: formTitle, description: formDescription, category_id: selectedCategoryId, questions }, null, 2)}</pre>
    </details>
