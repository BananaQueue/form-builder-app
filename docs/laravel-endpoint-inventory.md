...
- Frontend API calls are centralized through `src/apiBase.js`. Lookup/form reads, response reads/export, form writes (create/update/delete/duplicate), public submission, auth session (login/logout/session), the super-admin forms list, user management (list/create/delete/change-password), the audit log, and notifications (list/pending/acknowledge/mark-read) now use `/api/...`; the remaining banner calls still use `.php` compatibility names.
