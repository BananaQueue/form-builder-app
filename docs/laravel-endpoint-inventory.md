...
8. **Replace old PHP hardening tests**
   Done (hybrid). The runtime gaps that source-greps can't prove are now covered by phpunit feature tests: `SecurityHardeningTest` (login `password_verify`, rate-limit lockout, session regeneration + audit on success) and two new `BannerEndpointTest` cases (oversize and non-PNG rejection). CSRF enforcement stays a source-level guarantee because Laravel disables CSRF validation in the test environment. `api-tests/api-hardening.test.mjs` no longer greps the root `.php` files: its security assertions now inspect the Laravel source (`laravel/app/Http/Controllers/*`, `laravel/bootstrap/app.php` for the CSRF exemption allowlist, `laravel/config/app.php` for the timezone). The obsolete CORS-centralization test was dropped (Laravel serves same-origin, so there is no CORS layer). The app-file tests (schema dump, `vite.config.js`, `apiBase.js`, `LoginPage.jsx`) are unchanged.
...
9. **Retire root PHP endpoints**
   ...
   The e2e helpers (`loginViaApi`, `getFormFromApi`, `getUserFromApi`) and `tests/ownership-boundaries.spec.js` were converted to call the native `/api/...` routes, so no test outside the Laravel feature suite depends on the compatibility routes anymore.

   Deliberately kept: the `.php`-named **compatibility routes** in `laravel/routes/web.php` are Laravel routes (not XAMPP PHP), still aliasing the `LegacyXController` classes. They remain for rollback and are still load-bearing coverage for the Laravel feature tests: eight `*EndpointTest` files also assert the native routes, but `SaveFormEndpointTest`, `UpdateFormEndpointTest`, `DeleteFormEndpointTest`, and `SubmitResponseEndpointTest` exercise their controllers only through the `.php` route. Removing the compat routes was evaluated and declined -- it would force a ~12-file feature-test refactor (deleting redundant legacy methods and repointing the four form-write/submission suites) plus a CSRF `except`-list cleanup, for a purely cosmetic gain. It is not required for the Definition of Done, which is satisfied: the app runs on Laravel + MySQL with no XAMPP Apache serving PHP.
