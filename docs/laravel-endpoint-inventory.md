...
- Frontend API calls are centralized through `src/apiBase.js`. Lookup/form reads, response reads/export, form writes (create/update/delete/duplicate), public submission, auth session (login/logout/session), the super-admin forms list, and user management (list/create/delete/change-password) now use `/api/...`; remaining banner/notification/audit-log calls still use `.php` compatibility names.
