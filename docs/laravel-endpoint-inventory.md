...
- Frontend API calls are centralized through `src/apiBase.js`. Lookup/form reads, response reads/export, form writes (create/update/delete/duplicate), public submission, auth session (login/logout/session), the super-admin forms list, user management (list/create/delete/change-password), and the audit log now use `/api/...`; remaining banner/notification calls still use `.php` compatibility names.
...
7. **Move admin/user/banner/notification routes**
   ...
   Audit logs done: `get_audit_logs.php` is now aliased as `GET /api/admin/audit-logs` (same `LegacyAuditLogController::index`; a query-string read, so no path-id injection). `AuditLog.jsx` calls the native route. Compatibility route remains live for rollback and the `api-hardening` PHP-file inspection test. Notifications and banner remain.
